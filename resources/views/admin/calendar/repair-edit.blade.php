...
@section('title', 'Edit RT-' . str_pad($repairTicket->id, 4, '0', STR_PAD_LEFT))
@section('content')
...
.ep{padding:1.75rem 1.5rem 3rem;min-height:100vh;}
...
.ep-hero-left{display:flex;align-items:center;gap:1rem;position:relative;}
...
.btn-danger-soft{background:rgba(220,38,38,.15);color:#fca5a5;border:1px solid rgba(220,38,38,.3);}
.btn-danger-soft:hover{background:rgba(220,38,38,.25);}

/* Grid */
.ep-grid{display:grid;grid-template-columns:1fr 380px;gap:1.1rem;align-items:start;}
...
.card-head-action{margin-left:auto;font-size:.7rem;font-weight:600;color:var(--cyan);text-decoration:none;}
...
.f-error{font-size:.68rem;color:var(--red);margin-top:.25rem;}

/* Status badge read-only */
.status-display{display:flex;align-items:center;gap:.5rem;padding:.5rem .82rem;border-radius:9px;border:1.5px solid;height:2.35rem;}

/* Ref pill */
.ref-pill{display:inline-flex;align-items:center;gap:.4rem;font-size:.75rem;font-weight:700;padding:.3rem .85rem;border-radius:99px;border:1.5px solid var(--cyan-bd);color:var(--cyan);background:var(--cyan-bg);text-decoration:none;}
.ref-pill.emerg{border-color:var(--red-bd);color:var(--red);background:var(--red-bg);}
.ref-link{font-size:.7rem;font-weight:600;color:var(--cyan);text-decoration:none;display:inline-flex;align-items:center;gap:.25rem;transition:opacity .15s;}
.ref-link:hover{opacity:.7;}
.ref-link.emerg{color:var(--red);}
...
.ph-item{position:relative;width:78px;height:78px;border-radius:9px;overflow:hidden;border:2px solid var(--line);flex-shrink:0;transition:border-color .15s,transform .12s;}
...
.ph-item-pdf{width:78px;height:78px;border-radius:9px;background:var(--line2);border:2px solid var(--line);display:flex;flex-direction:column;align-items:center;justify-content:center;font-size:.6rem;font-weight:700;color:var(--ink3);text-decoration:none;gap:3px;transition:background .15s;flex-shrink:0;}
.ph-item-pdf:hover{background:var(--line);}
...
.ph-empty{text-align:center;padding:1.1rem;color:var(--ink4);font-size:.75rem;font-style:italic;border:1.5px dashed var(--line);border-radius:10px;}
...
.up-zone-title{font-size:.72rem;font-weight:600;color:var(--ink2);}
.up-zone-sub{font-size:.63rem;color:var(--ink4);margin-top:.18rem;}
.up-preview{margin-top:.6rem;display:none;flex-wrap:wrap;gap:.4rem;}
.up-thumb{position:relative;width:62px;height:62px;border-radius:7px;overflow:hidden;border:1.5px solid var(--line);}
...
.timeline-row{display:flex;gap:.65rem;padding:.6rem 0;border-bottom:1px solid var(--line2);}
.timeline-row:last-child{border-bottom:none;}
.timeline-dot{width:8px;height:8px;border-radius:50%;margin-top:5px;flex-shrink:0;}
.timeline-txt{font-size:.75rem;color:var(--ink2);line-height:1.4;}
.timeline-time{font-size:.65rem;color:var(--ink4);margin-top:2px;}

/* Save box */
.save-box{background:linear-gradient(135deg,#0c4a6e,#0891b2);border-radius:16px;padding:1.1rem 1.25rem;display:flex;align-items:center;gap:.85rem;}

/* Danger zone */
.danger-zone{background:var(--red-bg);border:1px solid var(--red-bd);border-radius:12px;padding:.85rem 1.1rem;display:flex;align-items:center;justify-content:space-between;gap:.75rem;flex-wrap:wrap;}
.btn-del-ticket{display:inline-flex;align-items:center;gap:.35rem;font-family:'Montserrat',sans-serif;font-weight:700;font-size:.75rem;padding:.42rem 1rem;border-radius:8px;background:var(--red);color:#fff;border:none;cursor:pointer;transition:filter .18s;}
.btn-del-ticket:hover{filter:brightness(1.1);}
...
@media(max-width:640px){.ep{padding:1rem .75rem 2.5rem;}.ep-title{font-size:1.1rem;}.f-2col{grid-template-columns:1fr;}}
</style>
...
<div class="container-xl px-0" style="max-width:1100px;">
    ...
    <div class="ep-hero">
        <div class="ep-hero-left">
            <div class="ep-hero-badge">
                <i class="fas fa-tools" style="color:#67e8f9;font-size:1.1rem;"></i>
            </div>
            <div>
                ...
                <div class="ep-sub">
                    @if($repairTicket->reference_type === 'job' && $repairTicket->jobRequest)
                        <span><i class="fas fa-briefcase" style="font-size:.6rem;"></i> {{ $repairTicket->jobRequest->job_number_name }}</span>
                    @elseif($repairTicket->emergency)
                        <span><i class="fas fa-exclamation-triangle" style="font-size:.6rem;"></i> {{ $repairTicket->emergency->job_number_name }}</span>
                    @endif
                    <span style="color:rgba(255,255,255,.15);">|</span>
                    <span><i class="fas fa-calendar" style="font-size:.6rem;"></i> {{ \Carbon\Carbon::parse($repairTicket->repair_date)->format('M d, Y') }}</span>
                </div>
            </div>
        </div>
        <div class="ep-actions">
            <a href="{{ route('superadmin.calendar.index') }}" class="btn-g btn-back">
                <i class="fas fa-arrow-left"></i> Back
            </a>
            <button type="button" class="btn-g btn-danger-soft" onclick="confirmDelTicket()">
    ...
    @if(session('repair_success'))
        <div class="flash-ok"><i class="fas fa-check-circle"></i> {{ session('repair_success') }}</div>
    @endif

    <form method="POST" action="{{ route('superadmin.repair.update', $repairTicket) }}"
        ...
        <div class="ep-grid">
            ...
            <div>
                ...
                <div class="card">
                    <div class="card-head" style="background:linear-gradient(to right,var(--line2),#fafbfd);">
                        <div class="card-head-icon" style="background:var(--line2);color:var(--ink3);">
                        ...
                        <div style="display:flex;align-items:center;gap:.75rem;flex-wrap:wrap;">
                            @if($repairTicket->reference_type === 'job' && $repairTicket->jobRequest)
                                <span class="ref-pill">
                                    ...
                                    {{ $repairTicket->jobRequest->job_number_name }}
                                </span>
                                <a href="{{ route('superadmin.calendar.index') }}" class="ref-link">
                                    <i class="fas fa-external-link-alt" style="font-size:.65rem;"></i> Open in calendar
                                </a>
                            @elseif($repairTicket->emergency)
                                <span class="ref-pill emerg">
                                    <i class="fas fa-exclamation-triangle" style="font-size:.7rem;"></i>
                                    {{ $repairTicket->emergency->job_number_name }}
                                </span>
                                <a href="{{ route('superadmin.calendar.index') }}" class="ref-link emerg">
                                    <i class="fas fa-external-link-alt" style="font-size:.65rem;"></i> Open in calendar
                                </a>
                            @else
                                <span style="font-size:.8rem;color:var(--ink4);">
                                    {{ ucfirst($repairTicket->reference_type) }} #{{ $repairTicket->reference_id }}
                                </span>
                            @endif
                        </div>
                    </div>
                ...
                <div class="card">
                    <div class="card-head" style="background:linear-gradient(to right,var(--cyan-bg),#fafbfd);">
                        <div class="card-head-icon" style="background:var(--cyan-bg);color:var(--cyan);">
                    ...
                    <div class="card-body">
                        ...
                        <div class="f-2col">
                            <div class="f-row">
                                <label class="f-label">Repair Date</label>
                                <input type="date" name="repair_date" class="f-ctrl"
                                       value="{{ \Carbon\Carbon::parse($repairTicket->repair_date)->format('Y-m-d') }}"
                                       required>
                                @error('repair_date')<div class="f-error">{{ $message }}</div>@enderror
                            </div>
                            <div class="f-row">
                                <label class="f-label">Status</label>
                                <select name="status" class="f-ctrl" required>
                                    @foreach(['open'=>'Open','in_progress'=>'In Progress','resolved'=>'Resolved','closed'=>'Closed'] as $val => $lbl)
                                        <option value="{{ $val }}" {{ $repairTicket->status === $val ? 'selected' : '' }}>
                                            {{ $lbl }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="f-row">
                            <label class="f-label">Damage Description <span style="color:var(--red);">*</span></label>
                            <textarea name="description" class="f-ctrl" required>{{ $repairTicket->description }}</textarea>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                                ...
                                @foreach($adminFotos as $index => $foto)
                                    ...
                                    @if(in_array($ext,['jpg','jpeg','png','gif','webp']))
                                        <div class="ph-item admin-item">
                                            ...
                                            <button type="button" class="btn-ph-del"
                                                    onclick="delPhoto({{ $repairTicket->id }},{{ $index }},this)"
                                                    title="Delete">
                                    ...
                                    @else
                                        <div class="ph-item admin-item" style="cursor:pointer;" onclick="window.open('{{ $url }}','_blank')">
                                            ...
                                            <span class="ph-badge admin">dmg</span>
                                            <button type="button" class="btn-ph-del"
                                                    onclick="event.stopPropagation();delPhoto({{ $repairTicket->id }},{{ $index }},this)">
                                                <i class="fas fa-times" style="font-size:9px;"></i>
                                            </button>
                                        </div>
                                    @endif
                        ...
                        <div class="ph-section-title" style="color:var(--cyan);">
                            <i class="fas fa-hard-hat" style="font-size:.65rem;"></i>
                        ...
                        {{-- Upload new admin photos --}}
                        <div class="ph-section-title" style="color:var(--ink3);margin-top:.5rem;">
                            <i class="fas fa-cloud-upload-alt" style="font-size:.65rem;"></i>
                            Add Damage Photos
                        </div>
                        <div class="up-zone" id="upZone"
                            ...
                            <div class="up-zone-title">Click or drag files here</div>
                            <div class="up-zone-sub">JPG, PNG, WEBP, PDF — multiple files allowed</div>
                            <input type="file" id="upInput" name="photos[]" accept="image/*,.pdf" multiple
                                   onchange="upHandleFiles(this.files)">
                        </div>
            ...
            <div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        @php
                            $stC = match($repairTicket->status){
                                'open'        => ['bg'=>'var(--amber-bg)','color'=>'var(--amber)','bd'=>'var(--amber-bd)','icon'=>'fa-exclamation-circle'],
                                'in_progress' => ['bg'=>'var(--purple-bg)','color'=>'var(--purple)','bd'=>'var(--purple-bd)','icon'=>'fa-tools'],
                                'resolved'    => ['bg'=>'var(--green-bg)','color'=>'var(--green)','bd'=>'var(--green-bd)','icon'=>'fa-check-circle'],
                                default       => ['bg'=>'var(--line2)','color'=>'var(--ink3)','bd'=>'var(--line)','icon'=>'fa-lock'],
                            };
                        @endphp
                        <div style="display:flex;align-items:center;gap:.85rem;padding:1rem;border-radius:12px;background:{{ $stC['bg'] }};border:1.5px solid {{ $stC['bd'] }};margin-bottom:1rem;">
                            <div style="width:40px;height:40px;border-radius:11px;background:rgba(255,255,255,.5);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                                <i class="fas {{ $stC['icon'] }}" style="font-size:1.1rem;color:{{ $stC['color'] }};"></i>
                            </div>
                            <div>
                                <div style="font-size:.8rem;font-weight:700;color:{{ $stC['color'] }};">
                                    {{ ucfirst(str_replace('_',' ',$repairTicket->status)) }}
                                </div>
                                <div style="font-size:.65rem;color:var(--ink3);margin-top:2px;">
                                    Updated {{ $repairTicket->updated_at->diffForHumans() }}
                                </div>
                            </div>
                        </div>
                ...
                {{-- Crew assignment --}}
                <div class="card">
                    <div class="card-head" style="background:linear-gradient(to right,var(--amber-bg),#fafbfd);">
                        <div class="card-head-icon" style="background:var(--amber-bg);color:var(--amber);">
                            <i class="fas fa-hard-hat"></i>
                        </div>
                        <h6>Crew Assignment</h6>
                    </div>
                    <div class="card-body">
                        @if($repairTicket->crew)
                            <div style="display:flex;align-items:center;gap:.75rem;padding:.75rem 1rem;border-radius:10px;background:var(--green-bg);border:1px solid var(--green-bd);margin-bottom:.85rem;">
                                <div style="width:34px;height:34px;border-radius:9px;background:var(--green);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                                    <span style="font-size:.7rem;font-weight:800;color:#fff;">
                                        {{ strtoupper(substr($repairTicket->crew->name,0,2)) }}
                                    </span>
                                </div>
                                <div>
                                    <div style="font-size:.82rem;font-weight:700;color:#14532d;">{{ $repairTicket->crew->name }}</div>
                                    <div style="font-size:.67rem;color:var(--green);margin-top:1px;">Currently assigned ✓</div>
                                </div>
                            </div>
                        @endif
                        <label class="f-label">Change Crew</label>
                        <select name="crew_id" class="f-ctrl">
                            <option value="">— Unassigned —</option>
                            @foreach(\App\Models\Crew::where('is_active', true)->orderBy('name')->get() as $crew)
                                <option value="{{ $crew->id }}"
                                    {{ $repairTicket->crew_id == $crew->id ? 'selected' : '' }}>
                                    {{ $crew->name }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="timeline-row">
                            ...
                            <div>
                                <div class="timeline-txt">Ticket created</div>
                                <div class="timeline-time">{{ $repairTicket->created_at->format('M d, Y · H:i') }}</div>
                            </div>
                        </div>
                        <div class="timeline-row">
                            <div class="timeline-dot" style="background:var(--cyan);"></div>
                            <div>
                                <div class="timeline-txt">Last updated · {{ $repairTicket->updated_at->diffForHumans() }}</div>
                                <div class="timeline-time">{{ $repairTicket->updated_at->format('M d, Y · H:i') }}</div>
                            </div>
                        </div>
                        <div class="timeline-row">
                            <div class="timeline-dot" style="background:var(--amber);"></div>
                            <div>
                                <div class="timeline-txt">Scheduled repair date</div>
                                <div class="timeline-time">{{ \Carbon\Carbon::parse($repairTicket->repair_date)->format('M d, Y') }}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                {{-- Danger zone --}}
                <div class="danger-zone" style="margin-top:1.1rem;">
                    <div>
                    ...
                    <button type="button" class="btn-del-ticket" onclick="confirmDelTicket()">
                        <i class="fas fa-trash"></i> Delete
                    </button>
                </div>
...
{{-- Delete form --}}
<form id="delTicketForm" method="POST"
      action="{{ route('superadmin.repair.update', $repairTicket) }}"
      style="display:none;">
    @csrf @method('DELETE')
</form>
...
function confirmDelTicket() {
    Swal.fire({
        title: 'Delete RT-{{ str_pad($repairTicket->id,4,"0",STR_PAD_LEFT) }}?',
...
function delPhoto(ticketId, index, btn) {
    Swal.fire({
        title: 'Delete this photo?', text: 'Cannot be undone.', icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#dc2626', cancelButtonColor: '#334155',
        ...
        try {
            const res  = await fetch(`/superadmin/repair-tickets/${ticketId}/photos/${index}`, {
                method: 'DELETE',
...
function upHandleFiles(files) {
    ...
    z.style.borderColor = 'var(--cyan)'; z.style.background = 'var(--cyan-bg)';
}
function upRenderThumb(file, idx) {
    const grid = document.getElementById('upPreview');
    grid.style.display = 'flex';
    const wrap = document.createElement('div'); wrap.id = 'up-th-' + idx; wrap.className = 'up-thumb';
    if (file.type === 'application/pdf') {
        wrap.innerHTML = `<div class="up-thumb-pdf"><i class="fas fa-file-pdf" style="font-size:1.1rem;color:#ef4444;"></i>${file.name.slice(0, 10)}</div>`;
    } else {
        const img = document.createElement('img'); img.src = URL.createObjectURL(file); wrap.appendChild(img);
    }
    const rm = document.createElement('button'); rm.type = 'button'; rm.className = 'btn-up-rm';
    rm.innerHTML = '&times;'; rm.onclick = () => upRemove(idx);
    wrap.appendChild(rm); grid.appendChild(wrap);
}
function upRemove(idx) {
    upFiles[idx] = null;
    const el = document.getElementById('up-th-' + idx); if (el) el.remove();
    if (upFiles.every(f => f === null)) {
        const z = document.getElementById('upZone'); z.style.borderColor = ''; z.style.background = '';
